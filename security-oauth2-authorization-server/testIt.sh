...
echo -e "######################################################################################################################################${NC}"
jsonCheck="$(curl -s -X POST  --header "Authorization: Basic Y2xpZW50SWRQYXNzd29yZDpzZWNyZXQ=" -d "token=dbe3d2c4-cb19-4ee5-b1bd-31666bb7e07d" $authorizationServer/oauth/check_token)"
if (( $? > 0 )); then
	echo " "
	echo "<<**********************************************************************************"
...
echo -e "${LGRAY}--------->>> ${YELLOW}OK! ${LGRAY}<<<---------${NC}"
echo -e "${LGRAY}--------->>> ${YELLOW}    ${LGRAY}<<<---------${NC}"
echo ""
echo ""
